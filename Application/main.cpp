...
void Program_Run ()
{
  ...
  char str[256];
	...
	gx3dObject *obj_tree, *obj_skydome, *obj_ground, *obj_mummy, *obj_ouch, *obj_explosion, * obj_sprits;
...
|___________________________________________________________________*/
  ...
  gx3dParticleSystem psys_blue = Script_ParticleSystem_Create("blue.gxps");
  ...
  gx3d_ReadLWO2File("Objects\\billboard_ghost.lwo",&obj_mummy,gx3d_VERTEXFORMAT_DEFAULT,gx3d_DONT_LOAD_TEXTURES);
  gx3d_ReadLWO2File("Objects\\ouch.lwo",&obj_ouch,gx3d_VERTEXFORMAT_DEFAULT,gx3d_DONT_LOAD_TEXTURES);
  gx3d_ReadLWO2File("Objects\\billboard_explosion.lwo",&obj_explosion,gx3d_VERTEXFORMAT_DEFAULT,gx3d_DONT_LOAD_TEXTURES);
  gx3d_ReadLWO2File("Objects\\billboard_spirits.lwo", &obj_sprits, gx3d_VERTEXFORMAT_DEFAULT, gx3d_DONT_LOAD_TEXTURES);
  ...
  gx3dTexture tex_mummy     = gx3d_InitTexture_File("Objects\\Images\\mummy_d512.bmp","Objects\\Images\\mummy_d512_fa.bmp",0);
  gx3dTexture tex_red_mummy = gx3d_InitTexture_File("Objects\\Images\\mummy_red_d128.bmp","Objects\\Images\\mummy_red_d128_fa.bmp",0);
  gx3dTexture tex_ouch      = gx3d_InitTexture_File("Objects\\Images\\ouch.bmp","Objects\\Images\\ouch_fa.bmp",0);
  gx3dTexture tex_explosion = gx3d_InitTexture_File("Objects\\Images\\explosion.bmp","Objects\\Images\\explosion_fa.bmp",0);
  gx3dTexture tex_spirits = gx3d_InitTexture_File("Objects\\Images\\ghosts.bmp", "Objects\\Images\\ghosts_fa.bmp", 0);

  const int NUM_MUMMIES = 2;
  gx3dVector mummyPosition[NUM_MUMMIES];
  float mummySpeed[NUM_MUMMIES];
  boolean mummyDraw[NUM_MUMMIES];
  gx3dSphere mummySphere[NUM_MUMMIES]; // bounding sphere of the mummy in world coordinates
  boolean mummyOnScreen[NUM_MUMMIES];

  mummyPosition[0].x = 0;
  mummyPosition[0].y = 0;
  mummyPosition[0].z = -20;
  mummySpeed[0] = 0.1f;
  mummyDraw[0] = true;

  mummyPosition[1].x = 15;
  mummyPosition[1].y = 0;
  mummyPosition[1].z = -20;
  mummySpeed[1] = 0.2f;
  mummyDraw[1] = true;

  int num_Mummies_touched = 0;

  const int MAX_OUCH = 20;
  gx3dVector ouchPosition[MAX_OUCH];
  int ouchTimer[MAX_OUCH];   // in milliseconds
  int ouchIndex = 0;         // index into ouch array to create next 'ouch'
  
  const int MAX_EXPLOSION = 10;
  gx3dVector explosionPosition[MAX_EXPLOSION];
  int explosionTimer[MAX_EXPLOSION];  // in milliseconds
  int explosionIndex = 0;             // index into explosion array to create next explosion
  const int EXPLOSION_DURATION = 500; // in milliseconds
...
|___________________________________________________________________*/
      ...
      // mouse left press?
        ...
        bool mummy_hit = false;
        int i;
        for (i=0; i<NUM_MUMMIES && !mummy_hit; i++) {
          if (mummyOnScreen[i]) {
            gxRelation rel = gx3d_Relation_Ray_Sphere(&viewVector,&mummySphere[i]);
            if (rel != gxRELATION_OUTSIDE) {
              // Play sound effect here for the collision
              // ADD CODE HERE
              // Remove this mummy from the game
              mummyDraw[i] = false;
              num_Mummies_touched++;
              if (num_Mummies_touched > 2)
                num_Mummies_touched = 2;

              // Create a new ouch
              ouchPosition[ouchIndex] = mummySphere[i].center;
              ouchTimer[ouchIndex] = 1000 + elapsed_time;   // draw this ouch for 1 second
        ...
        // Draw an explosion marker
        if (mummy_hit) {
          explosionPosition[explosionIndex] = mummySphere[i-1].center;
          explosionPosition[explosionIndex].y += 5;
        }
        else {
          // Compute a point 40 feet in front of the camera
          gx3dVector pos;
          pos.x = position.x + (heading.x * 40);
          pos.y = position.y + (heading.y * 40);
          pos.z = position.z + (heading.z * 40);
          explosionPosition[explosionIndex] = pos;
        }
        explosionTimer[explosionIndex] = EXPLOSION_DURATION + elapsed_time;
        explosionIndex = (explosionIndex + 1) % MAX_EXPLOSION;
      }
		}
...
|___________________________________________________________________*/
    ...
    if (gx3d_BeginRender ()) {                                             
      ...
      // Set new lighting for mummy
      gx3d_DisableLight(dir_light);
      gx3d_SetAmbientLight(color3d_white);

      // Alternate position of mummy
      for (int i=0; i<NUM_MUMMIES; i++) {
        mummyPosition[i].x += mummySpeed[i];
        //mummyPosition[i].z += (mummySpeed[i] * 0.8f); 
        if (mummyPosition[i].x > 20)
          mummySpeed[i] *= -1;
        else if (mummyPosition[i].x < -20)
          mummySpeed[i] *= -1;
      }
      // Update bounding spheres of each mummy
      for (int i=0; i<NUM_MUMMIES; i++) {
        if (mummyDraw[i]) {
          mummySphere[i] = obj_mummy->bound_sphere;
          mummySphere[i].center.x *= 6;
          mummySphere[i].center.y *= 6;
          mummySphere[i].center.y *= 6;
          mummySphere[i].center.x = mummyPosition[i].x;
          mummySphere[i].center.y = mummyPosition[i].y;
          mummySphere[i].center.z = mummyPosition[i].z;
          mummySphere[i].radius *= 6;
        }
      }
      ...
      for(int i = 0; i<NUM_MUMMIES; i++) {
        mummyOnScreen[i] = false;
        if (mummyDraw[i]) {
          // check the bounding volume of the mummy with the view frustum
          if (true) {
            gx3d_GetScaleMatrix (&m1, 6, 6, 6);
            gx3d_GetBillboardRotateYMatrix  (&m2, &billboard_normal, &heading);
			      gx3d_GetTranslateMatrix (&m3, mummyPosition[i].x, mummyPosition[i].y, mummyPosition[i].z);
			      gx3d_MultiplyMatrix (&m1, &m2, &m);
			      gx3d_MultiplyMatrix (&m, &m3, &m);
            gx3d_SetObjectMatrix (obj_mummy, &m);
            gx3d_SetTexture (0, tex_mummy);
            gx3d_DrawObject (obj_mummy, 0);
            mummyOnScreen[i] = true;
          }
        }
      }
      ...
      for (int i = 0; i<MAX_OUCH; i++) {
        if (ouchTimer[i] > 0) {
          ...
          gx3d_GetTranslateMatrix(&m3, ouchPosition[i].x, y+12, ouchPosition[i].z);
          gx3d_MultiplyMatrix(&m1,&m2,&m);
          gx3d_MultiplyMatrix(&m,&m3,&m);
      ...
      /*____________________________________________________________________
      |
      | Process explosion markers
      |___________________________________________________________________*/

      {
      static gx3dVector billboard_normal = {0,0,-1};
      const float EXPLOSION_SCALE = 10;
      // Update explosion timers
      for(int i = 0; i<MAX_EXPLOSION; i++)
        if(explosionTimer[i] > 0)
          explosionTimer[i] -= elapsed_time;
      ...
      for(int i = 0; i<MAX_EXPLOSION; i++) {
        if(explosionTimer[i] > 0) {
          gx3d_GetScaleMatrix(&m1,EXPLOSION_SCALE,EXPLOSION_SCALE,EXPLOSION_SCALE);
          gx3d_GetBillboardRotateXYMatrix(&m2,&billboard_normal,&heading);
          gx3d_GetTranslateMatrix(&m3,explosionPosition[i].x,explosionPosition[i].y,explosionPosition[i].z);
          gx3d_MultiplyMatrix(&m1,&m2,&m);
          gx3d_MultiplyMatrix(&m,&m3,&m);
          gx3d_SetObjectMatrix(obj_explosion,&m);

          gx3d_EnableTextureMatrix (0);
          int frame = (int) gx3d_Lerp (0, 15, ((float)EXPLOSION_DURATION - explosionTimer[i]) / EXPLOSION_DURATION);
          float offset[4] = { 0, 0.25, 0.5, 0.75 };
      ...
      //attach the particle system to the first mummy (index 0)
      gx3dVector firePosition = mummyPosition[0];
      gx3dVector bluePosition = mummyPosition[1];

      // Update the particle system's position to match the mummy's position
      gx3d_GetTranslateMatrix(&m, firePosition.x, firePosition.y, firePosition.z);
	  if (mummyDraw[0]) {
		  // Set the particle system's matrix
		  gx3d_SetParticleSystemMatrix(psys_fire, &m);
	  }
	  else {
		  // If the mummy is not drawn, set the particle system to a default position
          gx3d_GetTranslateMatrix(&m, -10, -10, -10);
          gx3d_SetParticleSystemMatrix(psys_fire, &m);
	  }
      gx3d_GetTranslateMatrix(&m, bluePosition.x, bluePosition.y, bluePosition.z);
      if (mummyDraw[1]) {
          // Set the particle system's matrix
          gx3d_SetParticleSystemMatrix(psys_blue, &m);
      }
      else {
          // If the mummy is not drawn, set the particle system to a default position
          gx3d_GetTranslateMatrix(&m, -10, -10, -10);
      ...
      gx3d_UpdateParticleSystem(psys_blue, elapsed_time);
      gx3d_DrawParticleSystem(psys_blue, &heading, draw_wireframe);
      ...
      if (num_Mummies_touched) {
        gx3d_DisableZBuffer();
        gx3d_EnableAlphaBlending();
        for (int i=0; i<num_Mummies_touched; i++) {
          ...
          gx3d_MultiplyMatrix(&m,&m3,&m);
          gx3d_SetObjectMatrix(obj_mummy,&m);
          gx3d_SetTexture (0, tex_red_mummy);
          gx3d_DrawObject(obj_mummy,0);
        }
        gx3d_DisableAlphaBlending();
        gx3d_EnableZBuffer();
